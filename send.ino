...
void loop() {
  ...
  // so the cpm burst guage does both, responsively and inversely proportional to counts. high counts means smaller time window, lower counts larger time window.
  else if (geigerCounter.GCMODE == 3) {
    detachInterrupt(GEIGER_PIN);

...
/*
what is a sensor node without a sensor? this rad sensor node is intended to receive impulses from a geiger counter, counting
the impulses from the geiger counter over each minute period with as small a granularity as main loop speed can manage.
*/
...
void radNodeSensor0() {
  ...
  */
  ...
  // check if impulse
  if (gcData.impulse == true) {
    gcData.impulse = false;
    if (radioData.broadcast == true) {
      // create transmission message
      memset(aesData.cleartext, 0, sizeof(aesData.cleartext));
      strcat(aesData.cleartext, aesData.fingerprint);
      strcat(aesData.cleartext, "IMP");
  ...
  gcData.precisionCounts = 0;
  memset(gcData.countsArrayTemp, 0, sizeof(gcData.countsArrayTemp));
  for (int i = 0; i < GCMAXCOUNT; i++) {
    if (gcData.countsArray[i] >= 1) { // only entertain non zero elements
    
      // compare current timestamp (per loop) to timestamps in array
      if (((timeData.timestamp - (gcData.countsArray[i])) > gcData.maxPeriod)) {
        gcData.countsArray[i] = 0;
        }
      else {
        gcData.precisionCounts++; // non expired counters increment the precision counter
        gcData.countsArrayTemp[i] = gcData.countsArray[i]; // store non expired
      }
    }
  }
  memset(gcData.countsArray, 0, sizeof(gcData.countsArray));
  memcpy(gcData.countsArray, gcData.countsArrayTemp, sizeof(gcData.countsArray));

  // then calculate usv/h
  gcData.CPM = gcData.precisionCounts;
  gcData.uSvh = gcData.CPM * 0.00332;

  if (radioData.broadcast == true) {
    // todo: broadcast periodically outside of value change
    if (gcData.CPM != gcData.previousCPM) {
      gcData.previousCPM = gcData.CPM;
      timeData.previousTimestampSecond = timeData.timestamp;
      // create transmission message
      memset(gcData.CPM_str, 0, sizeof(gcData.CPM_str));
      dtostrf(gcData.CPM, 0, 0, gcData.CPM_str);
      memset(aesData.cleartext, 0, sizeof(aesData.cleartext));
      strcat(aesData.cleartext, aesData.fingerprint);
      strcat(aesData.cleartext, "CPM");
      strcat(aesData.cleartext, gcData.CPM_str);
      // set our writing pipe each time in case we write to different pipes another time
      radio.stopListening();
      radio.flush_tx();
      radio.openWritingPipe(radioData.address[0][0]);
...
void loop() {
  ...
  /*
  optional because a sensor node does not have to receive but it can. for security reasons you may desire your sensor
...
void tubeImpulseISR() {
  gcData.impulse = true;
  if (gcData.countsIter < GCMAXCOUNT) {gcData.countsIter++;}
  else {gcData.countsIter=0;}
  gcData.countsArray[gcData.countsIter] = timeData.timestamp;
}
